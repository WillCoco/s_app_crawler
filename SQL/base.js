...
class Database {
  constructor(t = 'soul') {
    ...
    this.path = path.join(root, "SQL", this.name);
    this.db = db;
  }
  ...
  insertRows(infoList = [], postList = []) {
    return new Promise((resolve, reject) => {
      ...
      let stmt = this.db.prepare("INSERT INTO soul (userId, genderelation, mail, position, coordinate, userInfo, post) VALUES (?,?,?,?,?,?,?)");
  ...
  updateRows(info) {
    return new Promise((resolve, reject) => {
      // console.log("insertRows Ipsum i");
      let stmt = this.db.prepare("INSERT INTO soul (userId, registerTime, genderelation, mail, userInfo) VALUES (?,?,?,?,?)");

      const { userId, registerTime, genderelation, mail } = info;
      stmt.run([
        userId,
        registerTime,
        genderelation,
